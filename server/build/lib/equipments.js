...
const getAllEquipments = () => {
    return new Promise(async (resolve, reject) => {
        await connection.query('SELECT eq.name, eq.description, eq.updated_at, eqv.value, b.name FROM equipments eq, equipment_values eqv, brands b WHERE eq.id = eqv.equipment_id AND eq.brand_id = b.id', (err, rows) => {
            if (err) reject(err)
